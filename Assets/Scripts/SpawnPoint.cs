...
public class SpawnPoint : MonoBehaviour
{
    ...
    PlayerController target = GameManager.instance.players[0];
    ...
    private void GameModeType()
    {
        if(SinglePlayer == true)
        {
            ...
            //else
            //{
            //    Debug.LogError("ERROR: There can only be one single player mode.");
            //    Destroy(gameObject);
            //}
            if (PlayerOneSpawnZone == true)
            {
    ...
    private void SpawnPlayerOne()
    {
        ...
        GameObject.Instantiate(GameManager.instance.PlayerOne, /*GameManager.instance.SpawnLocations[spawn]*/ this.transform.position, Quaternion.identity);
    ...
    private void SpawnPlayerTwo()
    {
        ...
        // places our character at the random spawn location selected randomly
        GameObject.Instantiate(GameManager.instance.PlayerTwo, /*GameManager.instance.SpawnLocations[spawn]*/ this.transform.position, Quaternion.identity);
        

    }

    IEnumerator POneChangeName()
    {
        yield return new WaitForSeconds(3);
        target.name = "Player One";
        target.GetComponent<Camera>().name = "Player One Cam";
        GameManager.instance.players[0].GetComponent<Camera>().rect = new Rect(0f, .5f, 1f, .5f);
        //GameManager.instance.players[0].GetComponent<GameObject>().GetComponent<Camera>().rect = new Rect(0f, .5f, 1f, .5f);
        PlayerOneSpawned = true;
